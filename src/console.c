...
void fal_console_ui_display_interface(fal_console *console,
  ...
  case FAL_UI_POSITION_POP_BOX:
    ...
    y1 = (FAL_CONSOLE_HEIGHT / 2) - ((content_size + 2 + added_size) / 2);
    x2 = 3 * FAL_CONSOLE_WIDTH / 4;
    y2 = (FAL_CONSOLE_HEIGHT / 2) + ((content_size + 2 + added_size) / 2);
    background_color = FAL_COLOR_YELLOW;
    break;
  case FAL_UI_POSITION_BOTTOM:
    x1 = FAL_DISP_DESCRIPTION_BOX_LEFT;
    y1 = FAL_CONSOLE_HEIGHT - content_size - 3;
    x2 = FAL_DISP_DESCRIPTION_BOX_RIGHT;
    y2 = FAL_CONSOLE_HEIGHT - 2;
    break;
  }
...
void fal_console_ui_display_notice_pop(fal_console *console,
  ...
  va_start(va_args, content);
  vsprintf(c, content, va_args);
  va_end(va_args);
...
int fal_console_ui_display_pop(fal_console *console, const char *message,
  ...
  char pop_content[1024][FAL_CONSOLE_WIDTH];
  int pop_content_line_count =
      fal_split_displayed_text(message, '\\', pop_content);
  for (int i = 0; i < action_count; i++) {
    char choice[80] = {0};
    sprintf(choice, "[%s] %s", actions[i].key, actions[i].name);
    strcpy(pop_content[pop_content_line_count++], choice);
    pop_content_line_count++;
  }

  fal_console_ui_display_interface(console, FAL_UI_POSITION_POP_BOX,
                                   FAL_BORDER_SINGLE, pop_content,
                                   pop_content_line_count, false);

  bool choiceIsOkay = false;
  while (!choiceIsOkay) {
    enum fal_key user_choice = fal_platform_read_key();
    int i = 0;
    bool looped_through_choices = false;
    while (!looped_through_choices) {
      if (i >= action_count) {
        looped_through_choices = true;
        choiceIsOkay = false;
      } else {
        if (fal_key_from_string(actions[i].key) == user_choice) {
          looped_through_choices = true;
          choiceIsOkay = true;
          for (int action_to_run_idx = 0;
               action_to_run_idx < actions[i].action_count;
               action_to_run_idx++) {
            strcpy(actions_to_run[action_to_run_idx],
                   actions[i].actions[action_to_run_idx]);
          }
          return actions[i].action_count;
        } else {
          i++;
        }
      }
